/**
 * `arcane_status` — current state of projects and their containers
 * (plan phase 3.2).
 */
...
import type { ContainerSummary, ProjectDetails } from "../types.ts";
...
export function createStatusTool(): ToolDefinition<typeof parameters> {
	return defineTool({
		...
		description: [
			...
			"health, and the containers they are running (including published ports and",
			"public URLs).",
			"",
			"With no arguments, reports everything in the configured environment.",
			"The filter is a case-insensitive substring match.",
		].join("\n"),
		promptSnippet: "Check deployment status of Arcane projects and containers",
		parameters,
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			try {
				...
				return {
					...
					details: { environmentId, projects: matchedProjects, containers },
				};
			} catch (error) {
